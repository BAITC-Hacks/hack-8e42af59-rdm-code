...
import {authMode,ownProfile} from './auth';
export async function getData(user:User|null=null):Promise<AppData>{
 ...
 const [tasks,proposals,profiles]=await Promise.all([
  db.task.findMany({where:{OR:[{publicationStatus:'published'},...(isBusiness?[{ownerId:actorId}]:[])]},include:{owner:true,_count:{select:{proposals:true}}},orderBy:[{readinessScore:'desc'},{createdAt:'desc'}]}),
  ...
  db.user.findMany({where:{publicProfile:true,provider:authMode()},orderBy:{createdAt:'desc'}})
 ]);
 ...
 return {
  ...
  me:user?ownProfile(user):null,authMode:authMode(),aiMode:process.env.AI_MODE==='live'&&Boolean(process.env.AI_API_KEY)?'live':'mock'
 };
}