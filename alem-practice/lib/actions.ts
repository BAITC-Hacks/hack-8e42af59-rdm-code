...
export async function act(action:string,payload:unknown,actor:Actor){
 ...
 if(action==='createTask'){
  ...
  const text=p.draftText||p.fields.context||p.title;
  const sourceKey=createHash('sha256').update(JSON.stringify([actor.id,text,p.topic])).digest('hex');
  const draft=await db.draft.upsert({where:{sourceKey},update:{},create:{id:crypto.randomUUID(),businessId:actor.id,text,industry:p.topic,sourceKey}});
  return db.task.create({data:{ownerId:actor.id,draftId:draft.id,title:p.title,topic:p.topic,fields:JSON.stringify(p.fields),learning:JSON.stringify(p.learning)}});
 }
 if(action==='updateTask'){
  ...
  const changed=t.title!==p.task.title||t.topic!==p.task.topic||fieldKeys.some(k=>JSON.parse(t.fields)[k]!==p.task.fields[k])||t.learning!==JSON.stringify(p.task.learning);
  return db.task.update({where:{id:p.id},data:{title:p.task.title,topic:p.task.topic,fields:JSON.stringify(p.task.fields),learning:JSON.stringify(p.task.learning),confirmedFields:JSON.stringify(confirmed),readinessScore:calculateScore(p.task.fields,confirmed),...(changed?{publicationStatus:'unpublished',reviewRequired:true}:{})}});
 }
 if(action==='confirmTask'||action==='publishTask'){
  ...
  if(action==='confirmTask'){const confirmed=fieldKeys.filter(k=>validField(k,fields[k]));return db.task.update({where:{id},data:{confirmedFields:JSON.stringify(confirmed),readinessScore:calculateScore(fields,confirmed),reviewRequired:false}})}
  if(t.reviewRequired)throw new ActionError('После редактирования подтвердите сведения заново.');
 ...
 if(action==='createProposal'){
  ...
  if(!task)throw new ActionError('Задача не найдена',404);
  // Preserve the existing UI's demo links, but store new proposals with absolute HTTP URLs.
  const prototypeUrl=p.prototypeUrl.startsWith('/')?new URL(p.prototypeUrl,process.env.APP_URL||'http://127.0.0.1:3000').href:p.prototypeUrl;
  const submissionKey=createHash('sha256').update(JSON.stringify([actor.id,p.taskId,p.idea,p.plan,p.durationDays,prototypeUrl,p.assumptions||''])).digest('hex');
  const existing=await db.proposal.findUnique({where:{submissionKey}});if(existing)return existing;
  if(task.publicationStatus!=='published')throw new ActionError('Задача ещё не опубликована');
  return db.proposal.create({data:{...p,prototypeUrl,submissionKey,plan:JSON.stringify(p.plan),teamId:actor.id}});
 }
 if(action==='decideProposal'){
  ...
  const result=await db.proposal.update({where:{id:p.id},data:{status:p.status}});if(p.status==='selected')await db.milestone.upsert({where:{proposalId:p.id},update:{},create:{proposalId:p.id}});return result;
 }
 ...
 if(action==='confirmMilestone'){
  ...
  const current=await db.milestone.findUniqueOrThrow({where:{id},include:{award:true}});if(current.award)return {alreadyConfirmed:true,points:current.award.points};if(current.status!=='submitted')throw new ActionError('Команда ещё не отправила результат');await db.award.create({data:{milestoneId:id,teamId:m.proposal.teamId,points:50}});await db.team.update({where:{id:m.proposal.teamId},data:{practicePoints:{increment:50}}});await db.milestone.update({where:{id},data:{status:'confirmed',confirmedAt:new Date()}});return {points:50};
 }
